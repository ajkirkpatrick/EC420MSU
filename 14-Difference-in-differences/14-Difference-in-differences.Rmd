...
### We solved the selection bias problem by getting a good estimate of $E[Y_0|D=1]$, the counterfactual
We did this by using exogenous variation from $Z$ to instrument for $D$, giving us a $\hat{D}$ that is independent of the potential outcomes.

Since $Z$ is *as good as randomly assigned* (conditional on other exogenous $x$), then so is $\hat{D}$.
This is referred to as the *conditional independence assumption*, or *CIA*.
...
```{r echo=F, include=T, out.width='80%'}
knitr::include_graphics('MMfig51.png')
```
...
knitr::include_graphics('MMfig53.png')
```
...
```{r echo=F, include=T, out.width='85%'}
...
Where $\delta_{t}$ flexibly controls for a common time trend. This would result in $\beta_2$ being the $DID$ estimate.
...
```{r Trend3, echo=F, include=T, out.width='85%'}
knitr::include_graphics('MMfig56.png')
```
