...
```{r Wooldridge2, echo=F, include=T}
knitr::kable(head(df %>% dplyr::select(wage, educ)))
```
]

.pull-right[
```{r Wooldridge3, echo=F, include=T, warnings=F, out.width='110%'}
d
...
knitr::kable(ex[,c('Ynum','X')], digits=1, col.names = c('  Y  ','  X  '), align='cc') %>%
...
```{r exampleOLSplot, echo=F, include=T}
