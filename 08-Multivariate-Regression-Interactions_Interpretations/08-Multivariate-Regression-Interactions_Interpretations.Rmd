...
### Interpretation
- A .red[shift] in the intercept **relative to the base group**
  - Base group: $male$
  - Base group: $age<65$
...
If we run this in R (leaving out the "education" column, since those are words and we can't regress on words), we would get a coefficient for $education==College$ and $education==Graduate$
...
```{r AddVars, echo=T}
census.small = census[census$state=='South Carolina'|census$state=='Arizona'|census$state=='Nevada',c('lweekinc','state','educ', 'exper')]
...
```{r TimeFE, echo=F, include=T, out.width='95%'}
NN = 4
TT = 10
...
- On the next slide, GDP_HAT is the residual from regresing GDP on categorical Year and Country
  - Same for EXPORT_HAT
  
- These are called **two-way fixed effects**
...
```{r twowayFE, echo=F, include=T, out.width='90%'}
df = df %>%
...
- $condition$ is our group dummy ( $male$ or $age>65$ )
...
```{r SlopeShift1, echo=F, include=T, out.width='100%'}
NN = 60
...
ww = LEV[sample(1:3, size=NN, replace=T),] %>%
  ...
  dplyr::mutate(wage = meanwage + slope*exper + rnorm(NN, 0, 10)) %>%
...
```{r SlopeShift2, echo=F, include=T, out.width='100%'}
