---
title: "Multivariate Regression: Interpretation and inference"
subtitle: "EC420 MSU"
...
**HW2**
- .red[Due] this Sunday Feb 16th at 11:59pm
- I made a small update to Task 1.1(e) (changed the variables in a plot I ask you to make).

**Quiz 2**
- Average of **6.42**, Median of **7**
- Even if you think you have a wrong number, keep going!

```{r QuizStats, out.width='60%'}
knitr::include_graphics('Quiz2Stats.png')
```
...
$$y = \tilde{\beta}_0 + \tilde{\beta}_1 x_1  + u \text{ (the tilde here means "bad")}$$
...
$$\hat{\beta}_1 =  \frac{\widehat{Cov}(X,Y)}{\widehat{Var}(X)} =  \frac{\widehat{Cov}(\hat{v},Y)}{\widehat{Var}(\hat{v})}$$

And the same with $\hat{\beta}_2$, but with $\hat{w}$. 
...
And since $SSE + SSR = SST$, then $R^2 = \left(1 - \frac{SSR}{SST}\right)$
...
$\Rightarrow \hat{\beta}$ is unbiased estimate of $\beta$.
...
We took a look at $\beta$ up to this point:
...
### So we know our multivariate $\hat{\sigma}^2_u$. 
...
knitr::kable(sstdf, 'html', align=c('cc'))%>% kable_styling(bootstrap_options = 'basic')
...
```{r DegFC, echo=F, include=T, out.width='90%'}
df = tibble(F = 0:120,
                C = (F-32)*(5/9))
...
Note that this is the distribution of $y$ conditional on the $x$'s. All of the random variation comes from $u$. $x$'s conditionally shift the mean.
...
- $E[\hat{\beta}]$
- $Var(\hat{\beta})$
- That $\hat{\beta}$ is normally distributed
...
```{r WooldridgeHomoskedasticity, echo=F, include=T, fig.cap='Homoskedasticity (from Wooldridge)', out.width='75%'}
...
```{r WooldridgeHeteroskedasticity, echo=F, include=T, fig.cap='Heteroskedasticity (from Wooldridge)', out.width='75%'}
...
Note that we have squared the sum-of-squares total in the denominator. The numerator looks a little like covariance, but it's the covariance of squared terms.
