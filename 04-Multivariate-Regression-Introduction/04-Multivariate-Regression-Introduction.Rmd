...
- It *goes into the error term*. 
- Let's make a new variable called $\tilde{u} = \delta_1 ability_i$

$$wage_i = \beta_0 + \beta_1 educ_i + \underbrace{\delta_1 ability_i + u_i}_{\tilde{u}_i}$$

- Just a note: usually the $\sim$ over a coefficient or variable (like $\tilde{u}$) will indicate it is related to, but different from, the non- $\sim$ version.
...
If we think $E[ability|educ]>0$, then this violates the assumption that $E[\tilde{u}|X] = 0$

Then, $\frac{\Delta u}{\Delta ability} = \delta_1 \neq 0$. 

Looking at $\tilde{u}_i = \delta_1 ability_i + u_i$, we can see why $E[\tilde{u}|X]\neq 0$ and $\beta_1$ was biased.
...
```{r LoadWage, echo=F, include=T, out.width='200%'}
wage2 = read_dta(file=file.path('../Data/wage2.dta'))
...
The best fit is no longer a line, but a *plane* with a slope in the $x$ and the $y$ axis of $\beta_1$ and $\beta_2$
...
```{r Wage2, echo=F, include=T, out.width='150%'}
...
- $\delta_{rain}$ is the $temp$ that **is** explained by $rain$
...
$$y = \gamma_0 + \gamma_1 \hat{v} + \varepsilon$$

We used the $\hat{v}$ and left $x_{rain}$ out. $\hat{v}$ contains only the part of $x_{temp}$ that is not correlated with $x_{rain}$.
...
That is, we get the (unbiased) coefficient one would get from regressing $y = \tilde{\beta}_0 + \tilde{\beta}_{temp} x_{temp} + \tilde{\beta}_{rain} x_{rain} + \tilde{u}$ by first "partialing" $x_{rain}$ out of $x_{temp}$ then regressing what is left on $y$.
...
- $\hat{\beta}_1 = \hat{\tilde{\beta}}_1 + \hat{\beta}_2 \hat{\delta}_1$
...
```{r reg1, echo=F, include=T}

kable(summary(lm(wage ~ educ, wage2))$coef, digits=2)
```
$$wage = \beta_0 + beta_1 educ + u$$
```{r reg2, echo=F, include=T}

kable(summary(lm(wage ~ educ + exper, wage2))$coef, digits=2)
```
...
1. Including an irrelevant variable.
...
2. Omitting a relevant variable.
