...
Let's say we have a coefficient $\hat{\beta}_j = 5$ and $se(\hat{\beta}) = 2$
- We *know* how to calculate these from data

Let's say we have a $H_0: \beta_j = 0$

### Let's look at the distribution of $\hat{\beta}_j$
```{r meanvar1, echo=F, include=T, out.width='80%'}
...
The solid red line is our hypothesis. It is out in a tail, and it looks like an unlikely value to observe given our $\hat{\beta}_j$ and $se(\hat{\beta}_j)$, right?
...
```{r meanvar3, echo=F, include=T, out.width='80%'}
par(mai = c(1,1,0,1))
...
The red sections have total area: `r round(2*pnorm(0,5,2), 3)`

The red area is the p-value for a two-tailed test.
...
### The red area is the p-value. It is small because it is very unusual to observe something as extreme as 0 when $\hat{\beta}_j = 5$ and $\widehat{se}(\hat{\beta}_j)=2$
...
```{r stataplot2, echo=F, include=T, out.width = '80%'}
par(mai = c(1,1,0,1))
curve(dnorm(x, mean=stata_b1, sd=stata_se), from=stata_b1-(4*stata_se), to=stata_b1 + (4*stata_se), xlab = expression(beta[bedrooms]), ylab = expression('Prob beta'), lwd=2)
...
```{r stataplot3, echo=F, include=T, out.width = '80%'}
...
```{r stataplot4, echo=F, include=T, out.width = '80%'}
...
We could rewrite the equation so that we get a coefficient that is equivalent to $\beta_{jc} - \beta_{univ}$:
...
Here I should add the result of a hypothesis test where beta_j = beta_k (if not the test discussed in Wooldridge's example)
...
$$F = \frac{\frac{(SSR_{R} - SSR_{UR})}{q}}{\frac{SSR_{UR}}{N-K-1}}$$
...
$F$, unlike $t$, takes *two* degrees of freedom: the numerator $(q)$and the denominator $(N-K-1)$.
- $F\sim F_{q,N-K-1}$
...
```{r Fgraph, include=T, echo=F, out.width='50%'}
