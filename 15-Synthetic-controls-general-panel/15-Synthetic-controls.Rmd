...
### Implementation
- Usually with a pre-packaged routine in Stata (*synth*)
- Uses panel data 
User specifies:
- **One** treatment unit
...
### Implementation
- Stata then makes a guess at $w$, weights on each of the control units
...
- The pre-treatment "synthetic" outcome is:
...
- The distance between $y^{synth}_t$ and $y^{actual}_t$ is checked. Much like OLS, synth sums the squared difference:
  - $(y^{synth}_t - y^{actual}_t)^2 + (y^{synth}_{t+1} - y^{actual}_{t+1})^2 + \cdots)$
  
- Stata *guesses* at the weights over and over again until the smalled squared sum of errors is found.
  - OK, there is a method where it's not just random guesses, but that's the gist of it
  
- Observed covariates can be included as well, and get their own weights.

---
class: MSU
# Synthetic Control

Let's pull up the Kirkpatrick and Bennear paper, as well as the Abadie paper that we didn't end up reading...
